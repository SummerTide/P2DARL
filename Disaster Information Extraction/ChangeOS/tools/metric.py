...
class DamagePixelMetric(ConfusionMatrix):
    def __init__(self, num_classes, logdir=None, logger=None, class_names=None):
        super(DamagePixelMetric, self).__init__(num_classes)
    ...
    def compute_F_measure_per_class(confusion_matrix, beta=1.0):
        precision_per_class = DamagePixelMetric.compute_precision_per_class(confusion_matrix)
        recall_per_class = DamagePixelMetric.compute_recall_per_class(confusion_matrix)
    ...
    def compute_recall_per_class(confusion_matrix):
        sum_over_row = np.sum(confusion_matrix, axis=1)[1:]
        diag = np.diag(confusion_matrix)[1:]
    ...
    def compute_precision_per_class(confusion_matrix):
        ...
        sum_over_col = np.sum(cm, axis=0)[1:]
        diag = np.diag(confusion_matrix)[1:]
    ...
    def summary_all(self, dec=5):
        dense_cm = self._total.toarray() # [5, 5]

        F1_per_class = np.round(DamagePixelMetric.compute_F_measure_per_class(dense_cm, beta=1.0), dec)
        precision_per_class = np.round(DamagePixelMetric.compute_precision_per_class(dense_cm), dec)
        recall_per_class = np.round(DamagePixelMetric.compute_recall_per_class(dense_cm), dec)

        tb = pt.PrettyTable()
        if self._class_names:
            tb.field_names = ['name', 'class', 'f1', 'precision', 'recall']
            for idx, (f1, precision, recall) in enumerate(
                    zip(F1_per_class, precision_per_class, recall_per_class)):
                tb.add_row([self._class_names[idx], idx, f1, precision, recall])

        else:
            tb.field_names = ['class', 'f1', 'precision', 'recall']
            for idx, (f1, precision, recall) in enumerate(
                    zip(F1_per_class, precision_per_class, recall_per_class)):
                tb.add_row([idx, f1, precision, recall])
